...
const fakeBackend = () => {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (config && config.headers) {
          ...
          var config2 = {
            method: 'delete',
            url: 'http://38.242.236.227:3456/api/customer/delete/'+config.headers.user.id,
            headers: { 
  ...
  mock.onDelete(url.DELETE_NEWSLETTER).reply(config => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (config && config.headers) {
          var data = '';
          var config2 = {
            method: 'delete',
            url: 'http://38.242.236.227:3456/api/newsletters/delete/'+config.headers.newsletter.id,
            headers: { 
              'Authorization': 'Bearer '+JSON.parse(localStorage.getItem("authUser")).accessToken
            },
            data : data
          ...
          axios(config2)
          .then(function (response) {
            resolve([200, config.headers.newsletter])
          })
          .catch(function (error) {
            reject([400, "Cannot delete user"])
          });
  ...
  mock.onGet(url.GET_POLLS).reply(() => {
    return new Promise((resolve, reject) => {
      var config = {
        method: 'get',
  ...
  mock.onGet(url.GET_NEWSLETTERS).reply(() => {
    return new Promise((resolve, reject) => {
      var config = {
        method: 'get',
        url: 'http://38.242.236.227:3456/api/newsletters',
        headers: { 
          'Authorization': 'Bearer '+JSON.parse(localStorage.getItem("authUser")).accessToken
        }
      };
        ...
        axios(config)
          .then(function (response) {
            if (response.data) {
              ...
              reject([400, "Cannot get newsletter"])
            }
          })
          .catch(function (error) {
            reject([400, "Cannot get newsletter"])
  ...
  mock.onGet(url.GET_BETALL).reply(() => {
    return new Promise((resolve, reject) => {
      var config = {
        method: 'get',
